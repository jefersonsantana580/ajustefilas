...
# ---- Bloco de imagem pequena no topo direito ----
with st.sidebar:
    st.markdown('**🔧 Opções visuais**')
    logo_url = st.text_input('URL da imagem (opcional)')
    logo_file = st.file_uploader('Imagem no topo (PNG/JPG)', type=['png','jpg','jpeg'], key='logo_upl')
    logo_width = st.slider('Largura da imagem (px)', 64, 320, 120)

row_top = st.columns([6,1])
with row_top[1]:
